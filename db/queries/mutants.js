...
// create mutant
export async function createMutant(name, alias, status, powerDescription, biography,  imageUrl, createdBy) {
  const sql = `
  INSERT INTO mutants (name, alias, status, power_description, biography, image_url, created_by)
  VALUES ($1, $2, $3, $4, $5, $6, $7)
  RETURNING *
  `;
const { rows: [mutant] } = await db.query(sql, [name, alias, status, powerDescription, biography, imageUrl, createdBy]);
  return mutant;
}
